//! In-process channel swarm. **Not** the production transport. Tests only.
...
use community_protocol::{CapabilityProfile, P2PShardChunk, PeerMessage, TaskSpec};
...
/// Channel-based double. Production networking is [`crate::MeshSwarm`].
...
impl InMemorySwarm {
    ...
    pub async fn handle_incoming_message(&self, from_addr: Option<SocketAddr>, msg: PeerMessage) {
        match msg {
            ...
            _ => {
                debug!("in-memory swarm ignored message variant");
            }
        }
    }
...
mod tests {
    ...
    async fn test_p2p_registry_and_heartbeat() {
        ...
        registry
            .heartbeat(&NodeId::from_string("node-alpha"), 9500)
            .await;
        let peer = registry
            .get_peer(&NodeId::from_string("node-alpha"))
            .await
            .unwrap();
    ...
    async fn test_direct_p2p_swarm_activation_and_gossip() {
        ...
        let swarm_a = InMemorySwarm::new(id_a.clone(), profile_a.clone());
        let swarm_b = InMemorySwarm::new(id_b.clone(), profile_b.clone());
